(function($) {
	...
	$.backyard = function(options) {
		...
		var defaults = {
			url : 'BackyardServlet',
			socket_url : 'ws://localhost:8080/bywWeb/test',
			failover : true,
			websocket : true
		};
		...
		// set options to $backyard.options
		$.backyard.options = options;
	...
	function comet() {
		...
		comet_process = jQuery.ajax( {
			type : 'POST',
			url : $.backyard.options.url,
			dataType : 'json',
			data : {
				data : comet_json
			},

			error : function(xmlhr, texst) {
		...
		try {
			...
			jQuery.each(mes.messages, function(i, val) {
				// get channel
					var channel = getChannel(val.ch);

					// if channel exists
					if (channel != null) {
						// if callback function existst
						if (channel.fn != null) {
							channel.fn(val.data);
						}
					}
	...
	$.backyard.websocket= function() {

		var websocket = {
			url : $.backyard.options.socket_url,
			trys : 0,
			_connect : function() {

				// create new socket opject
			var socket = new WebSocket(this.url);

			// regist websocket callback function
		...
		try {
				...
				// get channel
					var channel = getChannel(data.ch);
	...
	function handshake() {
		...
		var handshake_json = "{fn:'handshake'}"
					...
					success : function(mes) {
						try {
							if (mes.status == "OK") {
								...
								var server_suppords_ws = mes.websocket == "true";
								var browser_suppords_ws = window.WebSocket != undefined;
								var setting_ws = $.backyard.options.websocket;

								if (server_suppords_ws && browser_suppords_ws
										&& setting_ws) {
									$.backyard.websocket($.backyard.options.socket_url);
								} else {
									comet();
								}
								client_id = mes.id;
	...
	$.backyard.listen = function(channel, callback) {
		var listen_json = "{fn:'listen', channel:{channel_name:'" + channel
				+ "'}}";
	...
	$.backyard.publish = function(ch, message) {

		// var publish_json = "{fn:'send',channel:{channel_name:'" + channel
		// + "'},data:" + message + "}";
		...
		jQuery.ajax( {
			...
			url : $.backyard.options.url,
			data : {
				data : "" + JSON.stringify(publish_json)
			},
			success : function() {
	...
	// this message can invoked from other js functions or from e.g dwr
	$.backyard.newMessage = function(ch, message) {
		// get channel by name
		var channel = getChannel(ch);
		// if channel have a callback function
		if (channel.fn != null) {
			channel.fn(message);
		}
	...
	function failover() {
		try {
			...
			if (new Date().getTime() - lastupdate > 120000
					|| comet_process.readyState > 3) {
				if (window.console) {
